...
const FieldSearch: React.FC<SearchButtonProps> = ({ label = "Search", bgColorClass = "bg-slate-800 hover:bg-slate-700", ...props }) => {

    const { isLoading } = props.context;

    const Spinner: React.FC = React.memo(() => (
        <svg aria-hidden="true" role="status" className="app-spinner" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    return <div title="Presiona Enter para buscar" className="relative">
        {props.child}
        <button
            style={{ padding: "2px" }}
